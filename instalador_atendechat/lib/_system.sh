...
#######################################
# verifica e atualiza Node.js para versão 20.x
# Arguments:
#   None
#######################################
system_check_nodejs() {
  ...
  if [ -z "$NODE_VERSION" ]; then
    echo ""
    echo -e "${WARNING_COLOR}⚠️  Node.js não encontrado!${RESET_COLOR}"
    echo ""
    log_message "WARNING" "Node.js não instalado"
    NEED_INSTALL=true
  elif [ "$NODE_VERSION" -lt 20 ]; then
    echo ""
    echo -e "${WARNING_COLOR}⚠️  Node.js v${NODE_VERSION} encontrado (Requer v20+)${RESET_COLOR}"
    echo ""
    log_message "WARNING" "Node.js v${NODE_VERSION} incompatível (requer v20+)"
    NEED_INSTALL=true
  else
    echo ""
    echo -e "${SUCCESS_COLOR}✅ Node.js v${NODE_VERSION} OK${RESET_COLOR}"
    echo ""
    log_message "INFO" "Node.js v${NODE_VERSION} compatível"
    NEED_INSTALL=false
  fi
  ...
  if [ "$NEED_INSTALL" = true ]; then
    echo ""
    echo -e "${INFO_COLOR}📦 Instalando Node.js 20.x LTS...${RESET_COLOR}"
    echo ""
    
    log_message "INFO" "Iniciando instalação do Node.js 20.x"
    
    # Remover versões antigas do Node.js
    run_with_check "Limpeza de versões antigas do Node.js" \
      "sudo apt-get remove -y nodejs npm"
    
    # Adicionar repositório NodeSource para Node.js 20.x
    run_with_check "Download do script de instalação NodeSource" \
      "curl -fsSL https://deb.nodesource.com/setup_20.x -o /tmp/nodesource_setup.sh"
    
    run_with_check "Configuração do repositório NodeSource" \
      "sudo bash /tmp/nodesource_setup.sh"
    
    # Instalar Node.js 20.x
    run_with_check "Instalação do Node.js 20.x" \
      "sudo apt-get install -y nodejs"
    
    # IMPORTANTE: Limpar cache de comandos do bash para pegar nova versão
    hash -r
    
    # Usar caminho completo para garantir que pegamos a versão correta
    NODE_VERSION=$(/usr/bin/node --version 2>/dev/null | cut -d'v' -f2 | cut -d'.' -f1)
    NPM_VERSION=$(/usr/bin/npm --version 2>/dev/null | cut -d'.' -f1)
    
    # Se ainda não encontrou, tentar sem caminho completo após hash -r
    if [ -z "$NODE_VERSION" ]; then
      NODE_VERSION=$(node --version 2>/dev/null | cut -d'v' -f2 | cut -d'.' -f1)
    ...
    if [ -z "$NODE_VERSION" ] || [ "$NODE_VERSION" -lt 20 ]; then
      error_exit "Falha ao instalar Node.js 20.x. Versão atual: ${NODE_VERSION}" "Instalação Node.js" "1"
    fi
    
    echo ""
    echo -e "${SUCCESS_COLOR}✅ Node.js v${NODE_VERSION} instalado com sucesso!${RESET_COLOR}"
    echo -e "${SUCCESS_COLOR}✅ npm v${NPM_VERSION} instalado com sucesso!${RESET_COLOR}"
    echo ""
    
    log_message "SUCCESS" "Node.js v${NODE_VERSION} e npm v${NPM_VERSION} instalados"
    
    # Atualizar npm para versão mais recente
    echo ""
    echo -e "${INFO_COLOR}📦 Atualizando npm...${RESET_COLOR}"
    echo ""
    
    run_with_check "Atualização do npm" \
      "sudo npm install -g npm@latest"
    
    NPM_VERSION=$(npm --version 2>/dev/null)
    echo ""
    echo -e "${SUCCESS_COLOR}✅ npm v${NPM_VERSION} atualizado!${RESET_COLOR}"
    echo ""
    
    log_message "SUCCESS" "npm v${NPM_VERSION} atualizado"
  fi
...
system_git_clone() {
  ...
  log_message "INFO" "Iniciando clone do repositório Git"
  
  # Clonar em diretório temporário com verificação
  run_with_check "Clone do repositório Git" \
    "sudo su - deploy -c 'git clone ${link_git} /tmp/atendechat_temp_${instancia_add}'"
  
  # Verificar se pasta api_oficial existe
  check_dir_exists "/tmp/atendechat_temp_${instancia_add}/api_oficial" "Pasta api_oficial no repositório"
  
  # Criar pasta da instância
  run_with_check "Criação da pasta da instância" \
    "sudo su - deploy -c 'mkdir -p /home/deploy/${instancia_add}'"
  ...
  check_dir_exists "/home/deploy/${instancia_add}/backend" "Pasta backend"
  ...
  check_dir_exists "/home/deploy/${instancia_add}/frontend" "Pasta frontend"
  check_file_exists "/home/deploy/${instancia_add}/frontend/package.json" "package.json do frontend"
  
  # Limpar pasta temporária
  run_with_check "Limpeza de arquivos temporários" \
    "sudo su - deploy -c 'rm -rf /tmp/atendechat_temp_${instancia_add}'"
  
  log_message "SUCCESS" "Clone do repositório concluído com sucesso"
...
system_update() {
  ...
  sudo su - root <<EOF
  apt -y update
  sudo apt-get install -y uuid-dev libossp-uuid-dev libxshmfence-dev libgbm-dev wget unzip fontconfig locales gconf-service libasound2 libatk1.0-0 libc6 libcairo2 libcups2 libdbus-1-3 libexpat1 libfontconfig1 libgcc1 libgconf-2-4 libgdk-pixbuf2.0-0 libglib2.0-0 libgtk-3-0 libnspr4 libpango-1.0-0 libpangocairo-1.0-0 libstdc++6 libx11-6 libx11-xcb1 libxcb1 libxcomposite1 libxcursor1 libxdamage1 libxext6 libxfixes3 libxi6 libxrandr2 libxrender1 libxss1 libxtst6 ca-certificates fonts-liberation libappindicator1 libnss3 lsb-release xdg-utils
EOF
...
system_node_install() {
  print_banner
  printf "${WHITE} 💻 Instalando nodejs 14.x e npm 9.6.2+...${GRAY_LIGHT}"
  printf "\n\n"
